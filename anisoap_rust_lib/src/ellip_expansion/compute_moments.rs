use crate::calc_utils::simple_linalg::mat33_sym_inverse;
...
pub fn compute_moments_rust(
    ...
    if dil_mat.shape() != &[3, 3] {
        return Err("Dilation matrix needs to be 3x3".into());
    }

    // Check if the dilation matrix is symmetric
    for i in 0..3 {
        for j in 0..3 {
            if (dil_mat[[i, j]] - dil_mat[[j, i]]).abs() >= 1e-10 {
                return Err("Dilation matrix needs to be symmetric".into());
            }
        }
    }
    ...
    compute_moments_rust_unchecked(dil_mat, gau_cen, max_deg)
}
...
pub fn compute_moments_rust_unchecked(
    ...
    let (det, [inv00, inv01, inv02, inv11, inv12, inv22]) = mat33_sym_inverse(&dil_mat)?;
