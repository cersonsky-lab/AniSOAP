...
from anisoap.utils import TrivariateMonomialIndices, assert_close, spherical_to_cartesian
...
def spherical_to_cartesian_exact(lmax, num_ns):
    ...
    assert len(num_ns) == lmax+1, "Provided list of nmax values has to match number of l values"
    ...
    for n in range(num_ns[1]):
        ...
        for idx, n0, n1, n2 in monomial_indices:
            coeff = fac_n/factorial(n0)/factorial(n1)/factorial(n2)
            T[1][0,n,2*n0,2*n1+1,2*n2] = coeff # m=-1 with solid harmonic = y
            T[1][1,n,2*n0,2*n1,2*n2+1] = coeff # m=0 with solid harmonic = z
            T[1][2,n,2*n0+1,2*n1,2*n2] = coeff # m=1 with solid harmonic = x
    if lmax == 1:
        return T
...
def test_compare_against_exact(lmax, num_n):
    ...
    T_ref = spherical_to_cartesian_exact(lmax=2, num_ns=[num_n,num_n,1]) # reference values 
...
class TestSumOfCoefficients:
    ...
    def test_sum_for_l0(self, num_n):
        ...
        for n in range(num_n):
            ...
            assert_close(sum_of_all, 3.**n, rtol=1e-14)
