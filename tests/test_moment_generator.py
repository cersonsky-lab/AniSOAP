...
class TestMomentsTrivariateGaussian():
    ...
    def test_moments_diagonal_vs_exact(self, principal_components, center, maxdeg):
        ...
        moments_diagonal = compute_moments_diagonal_inefficient_implementation(principal_components, center, maxdeg)
            ...
            for n1 in range(maxdeg+1):
                for n2 in range(maxdeg+1):
                    ...
                    if deg > maxdeg:
                        ...
                        assert_close(moments_diagonal[n0,n1,n2], moments_diagonal[n1,n0,n2])
                        assert_close(moments_diagonal[n0,n1,n2], moments_diagonal[n2,n1,n0])
                        assert_close(moments_diagonal[n0,n1,n2], moments_diagonal[n0,n2,n1])
    ...
    As = [np.array([[1.,3,5.],[3.,7.,9.],[5.,9.,13.]]) + 8.1]
    As.append( np.diag([2.1,3.2,4]) + 1e-1 * np.array([[1,2,3],[2,1,4],[3,4,2.1]]))
    @pytest.mark.parametrize("maxdeg", maxdegs_small)
    @pytest.mark.parametrize("center", centers)
