...
def pairwise_aniso_expansion(lmax,
                             neighbor_list, 
                             species, 
        ...
        for neighbor_species in species:
            if (center_species, neighbor_species) in neighbor_list.keys:
                ...
                for isample, nl_sample in enumerate(nl_block.samples):
                    ...
                    r_ij = np.asarray([nl_block.values[isample,0],
                                       nl_block.values[isample,1],nl_block.values[isample,2]]).reshape(3,)
...
#                    print(l, np.asarray(values_ldict[l]).shape, len(nl_block.samples))
    ...
    pairwise_aniso_feat = TensorMap(Labels(["species_center", "species_neighbor", "angular_channel"],np.asarray(keys,
                                                                                                                dtype=np.int32)),
                                    tensorblock_list)
...
def contract_pairwise_feat(pair_aniso_feat, species):
    ...
    """ 
    aniso_keys = list(set([tuple(list(x)[:1]+list(x)[2:]) for x in pair_aniso_feat.keys]))
    ...
    for key in aniso_keys:
        ...
        for ele in species:
            ...
            sel_blocks = [pair_aniso_feat.block(i) for i in blockidx if key==tuple(list(pair_aniso_feat.keys[i])[:1]
...
#             num_ns.append(max_angular + 1 - l)
    ...
    def transform(self, frames, show_progress=False):
        ...
        for n in range(1,num_frames):
            self.frame_to_global_atom_idx[n] = self.num_atoms_per_frame[n-1] + self.frame_to_global_atom_idx[n-1]
        ...
        pairwise_aniso_feat = pairwise_aniso_expansion(self.max_angular, self.nl, species, self.frame_to_global_atom_idx,
                                                       rotation_matrices, ellipsoid_lengths, self.sph_to_cart,
                                                       self.radial_basis)
