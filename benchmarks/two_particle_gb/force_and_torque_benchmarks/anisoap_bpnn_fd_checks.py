#!/usr/bin/env python3
"""
Finite-difference and autograd checks for AniSOAP-BPNN.

This script does three things:

1. Trains an AniSOAP-BPNN energy model with energy + position-gradient targets.
2. Checks energy parity with the exported model.
3. Checks forces in two different ways:
...
def build_hypers(lmax: int = 6, nmax: int = 9) -> dict:
...
def load_dataset(path: Path, stride: int = 18) -> tuple[list, list]:
    ...
    # Keep the original frames untouched so their SinglePointCalculator still
    # contains energy/forces. Zero cells only on copies used for System creation.
    frames_for_systems = []
    for frame in frames:
...
def build_trainer(num_epochs: int, batch_size: int, learning_rate: float) -> Trainer:
    """Build the AniSOAP-BPNN trainer."""
    return Trainer(
        {
            "regularizer": 1e-3,
    ...
    try:
        ...
        for sample in dataset:
            ...
            if not energy.requires_grad:
                raise RuntimeError(
                    "Non-exported model energy does not require grad even in train mode. "
                    "This means the forward path is detaching descriptor or head values. "
            ...
            forces_true = true_forces(sample).to(forces_pred)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train AniSOAP-BPNN and run finite-difference/autograd checks."
    )
    parser.add_argument(
        "--xyz",
        type=Path,
...
def main() -> None:
    ...
    hypers = build_hypers(lmax=2, nmax=2)
