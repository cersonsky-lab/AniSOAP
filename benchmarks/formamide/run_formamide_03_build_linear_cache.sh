#!/usr/bin/env bash
set -eu

mkdir -p publication_linear_cache publication_linear_runs/logs

python -u fit_lr_energy_force_torque.py \
  --cache-output publication_linear_cache/formamide_n10_l10_rcut5_rg2p5_dscale0p7.npz \
  --train-input publication_splits/formamide_train.xyz \
  --validation-input publication_splits/formamide_valid.xyz \
  --test-input publication_splits/formamide_test.xyz \
  --max-angular 10 \
  --max-radial 10 \
  --cutoff 5.0 \
  --radial-width 2.5 \
  --diameter-scale 0.7 \
  --basis-rcond 1e-6 \
  --basis-tol 1e-2 \
  --position-step 1e-4 \
  --rotation-step 1e-4 \
  --feature-batch-size 128 \
  --quaternion-order wxyz \
  --quaternion-matrix-direction body_to_space \
  --torque-target-frame space \
  | tee publication_linear_runs/logs/03_build_linear_cache.log
