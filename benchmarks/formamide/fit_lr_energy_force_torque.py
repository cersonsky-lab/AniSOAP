...
def apply_diameter_scale(frames, diameter_scale):
    """Scale AniSOAP ellipsoid diameters in-place.

    This is intended for descriptor-hyperparameter studies where the CG
    ellipsoid lengths are scaled at runtime without writing modified XYZ files.
    It must be applied before feature and descriptor-derivative construction.
    """
    if diameter_scale == 1.0:
        return frames

    keys = ("c_diameter[1]", "c_diameter[2]", "c_diameter[3]")

    for frame_index, frame in enumerate(frames):
        for key in keys:
            if key not in frame.arrays:
                raise KeyError(
                    f"Cannot apply --diameter-scale because frame {frame_index} "
                    f"is missing {key}. Available arrays: {sorted(frame.arrays)}"
                )
            frame.arrays[key] = frame.arrays[key].astype(float, copy=True) * diameter_scale
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "Scale c_diameter[1], c_diameter[2], and c_diameter[3] "
            "arrays before AniSOAP feature construction."
        ),
    )
...
def _read_required_frames(
    ...
    frames = apply_diameter_scale(frames, args.diameter_scale)
...
def main() -> None:
    ...
    if args.cache_input is not None and args.diameter_scale != 1.0:
        raise ValueError(
            "--diameter-scale cannot be used with --cache-input because "
            "the cached AniSOAP features and derivatives have already been computed. "
            "Use --diameter-scale only when building a new cache."
        )
