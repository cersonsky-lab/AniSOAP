...
def apply_diameter_scale(frames, diameter_scale):
    """Scale AniSOAP ellipsoid diameter arrays in-place."""
...
def run_explicit_split_mode(args: argparse.Namespace) -> None:
    ...
    if args.diameter_scale != 1.0:
        print(f"applying diameter scale: {args.diameter_scale}")
        train_frames = apply_diameter_scale(train_frames, args.diameter_scale)
        validation_frames = apply_diameter_scale(validation_frames, args.diameter_scale)
        test_frames = apply_diameter_scale(test_frames, args.diameter_scale)
